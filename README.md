...
『 * 』->稱為依址取值運算子，用來取得指標所指向記憶體位址中的``值（內容）``，後方運算元為一個``記憶體位址``  
ex:  
```
int *ptr,a=5;
ptr=&a;
...
open(檔案名稱，開啟模式;)
``
ps 1:檔案名稱是想開啟的檔案名稱，也可以包含檔案路徑位置。開啟模式有下列幾種:ios::in->輸入模式, ios::out->寫入模式, ios::ate->從檔案尾端輸入輸出, ios::app->在檔案尾端繼續寫入, ios::trunc->若檔案已存在則清除檔案內容, ios::binary->以二進位模式開啟檔案。若傳入的參數超過兩個可用``|``分開  
...
```
ps:除了用get和put函式來讀寫檔案，C++還提供了利用``串流方式``來進行檔案讀寫，就像使用cin和cout容易。使用其內建的函式，不需要開啟(open)檔案，就可以進行串流的檔案讀寫  
```
ex:
...
之後就可以使用串流讀寫檔案 
F1>>mystring; // >>為析取器，也就是從標準輸出流中讀取一個指定型別資料
F2<<"hello"<<endl; // <<為插入器，由向流輸出資料到檔案
```
...
***
#### ***動態記憶體配置***    
假設現在必須紀錄不明大小的物件，並分析這些物件，首先必須將所有物件獨進記憶體中，但不到執行期間，不會知道，到底一次有多少物件被讀入，因此需要動態配置記憶體機制，有了動態記憶體的配置，程式可在執行期間向作業系統，動態指定需要多少記憶體，而這些記憶體在程式釋放空間之前，並不會被其他程式所使用，以增加記憶體使用效率  
 
* ***動態配置記憶體函式介紹***  

(1) new 運算子:配置動態記憶體  
new會向作業系統要求記憶體，函式宣告如下:  
```
指標= new 型態; 
      OR 
指標=new 型態[數量];
```
ps:第一種，new會向作業系統要求一塊大小為型態大小的記憶體空間，之後回傳指向該記憶體的指標。在第二種會回傳連續多個大小為型態大小的記憶體空間，之後回傳指向該記憶體指標  

(2) delete 運算子:釋放記憶體空間，函式宣告如下:  
```
delete 指標;
     OR 
delete [] 指標;
```
ps:delete會釋放一個指向程式配置的記憶體指標，每當使用new配置一塊新的記憶體空間，當此空間使用完畢後，除非程式結束，系統並不會自動歸還記憶體，因此必須要在記憶體空間使用完畢後，使用delete來歸還記憶體空間回系統  
...
ii. private->私有的。此標籤下就是標記為私有，表示只有在這個class內的方法可以存取，才可以使用這些變數或函式，或是此class的「朋友(friend)」也可以  
...
ps:public下的所有東西都可以被其他函數直接呼叫，而Private標籤中必須使用Public中有定義到的member function 才能讀取。可以避免資料不小心被其他函數破壞
```
...
ps:利用C++模擬堆疊操作，最簡單是使用一個陣列，以及使用一個變數來記錄目前堆疊的頂端位置(堆疊內假若目前尚無元素，習慣性設定為-1)  
...
ps:函式s1呼叫了s2函式，s2函式呼叫了s3函式，若將這些函式視為一個個元素。上述是說明s1函式會等待s2函式完成工作後，才會執行s1函式的相關程式碼，同理，s2也會等待s3完成工作後，再執行其他部分
```
...
ps:使用C++模擬佇列操作，最簡單的方式也是使用一個陣列，當對佇列做enqueue和dequeue命令時，需要維護兩資訊:佇列的前端(front)及尾端(rear)，因此我們可以將陣列及紀錄前後端的變數，使用結構(structure)來包裝起來  
ex:
```
struct queue{  
int q[100];
...
struct node{
int data; //整數
struct node *next; //指標指向下一個node的宣告
};
ps:上方定義單向串列的一個節點，node結構第一個成員data，可根據所要儲存的資料類型更改資料型態，串列程式通常會使用一個指標指向串列開頭，若是有一個節點next指標為NULL，則可判斷為串列結尾
```
* 配置新節點可以使用動態配置
```
...
head->next=new NODE;
head->next->next=NULL;
ps:第一行使用typedef使得struct node可以被NODE取代。第二行使用new函數配置一塊新的記憶體，並回傳記憶體位址指定給指標head，作為串列第一個元素。第三行與第二行類似，產生串列第二個元素。第四行則將節點下一個元素設為NULL，表示串列結尾在此節點
```
```
串列的優點:  
