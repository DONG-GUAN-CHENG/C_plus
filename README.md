...
``物件導向設計``是以系統所要處理的物件，作為系統架構的出發點，會先從抽象的層面來分析要處理的類別(Class）的資料，然後思考各種類別的資料，會以哪些方法(Method)來操作，然後最後使用繼承來表現其共同點。而整個系統是由各個封裝良好的物件模組所架構起來，每一個物件模組就是一個類別定義。它包含一組私有的資料結構，以及開放給外部還操作這組資料的方法，以``模組化``為基本精神，用封裝、繼承、多型...等方式為原則，來產生具有可重用性、可維護性以及可擴展性的軟體
***
#### ***資料資料型別與計算***
* ***C++程式架構***  
...
(1) 資料成員：可以在類別中定義多種資料型態的變數，如int、char、float、string  
(2) 成員函式：類別中還包括存取成員的函式  
...
類別和結構的差別在於class也可以包含「函式」，而class中的標籤有三種保留字:  
...
iii. protected->保護的。介於兩者之間，為同類別和具有繼承關係的類別具有存取權限  
...
* ***類別的基本使用***  
要使用class就像宣告其他型態變數一樣  
```
ex:
Cuboid cuboid1;  
cuboid1.set_value(1,2,3);
cuboid1.volume();
ps:以上成功宣告了一個名為cuboid1的Cuboid物件，當我們要使用這個物件的內容函式時，利用「.」來呼叫
```
定義class內函式的方式有兩種:一種是直接在class內宣告該函式時定義。另一種是在class之外定義  
```
...
其中``建構子(constructor)``作用就是在變數被創造時所執行的函式，若沒有定義任何建構子，系統會自動使用初始建構子  
